...
public class LazyToSql {
    public String[] tableSqlCommand(String filterTemp, String globalFilterCommand, String query, String sortTemp, boolean sortByIsEmpty, LazyService service) {
        ...
        boolean where = false;
        if (!service.isFilter() && service.filterGlobal) {
            where = true;
            filterTemp = " Where " + filterTemp;
        }
        if (query.length() > 0 && !filterTemp.contains("Where"))
            filterTemp += " Where " + query + " ";
        else if (query.length() > 0) {
            filterTemp += " and " + query;
        }
        /*else
